...
const AdoptModal = ({
  ...
  const adoptPet = async () => {
    ...
    try {
      ...
      setTimeout(() => (window.location.href = "/mypets"), 1000);
  ...
  const ErrorText = (
    <p className="modal__text--error">You already have this pet adopted!</p>
  );
  const ErrorText2 = (
    <p className="modal__text--error">Someone already owns this pet</p>
  );
  const SuccessText = (
    <>
      <p className="modal__text--success">
        Congratulations!! Pet Successfully adopted.
      </p>
    </>
  );
  ...
  return (
    <Modal isOpen={modalIsOpen} onRequestClose={toggleModal} className="modal">
      <div className="flex-col-1">
        ...
        <p className="modal__text mb-1">
          {petBreed}s require a lot of attention and care!!
        </p>
        {error}
        {message}
