...
const PetPage = ({ pets, profile, setProfile }) => {
  ...
  const checkIfAdopted =
    index === -1 ? false : pets[index].status.toLowerCase().includes("adopted");
  ...
  const savePet = async () => {
    const settings = {
      ...
      body: JSON.stringify({ pet: pets[index].name, id: pets[index]._id }),
    };
    try {
      const response = await fetch(
        `${BASE_URL}/savepet/${profile._id}`,
        settings,
      );
      const data = await response.json();
      console.log(data);
      toast.success(`   Pet saved! ❤️`);
      setProfile(data);
    } catch (e) {
      console.log(e);
    }
        ...
        <div className="pet-page__right">
          ...
          <div className="button-wrapper">
            <button
              onClick={toggleModal}
              className={
                checkIfAdopted ? "pet-page__adopt disabled" : "pet-page__adopt"
              }
          ...
          <div className="button-wrapper col-end">
            ...
            <button onClick={savePet} className="pet-page__btn">
              Save <BsPlusCircle />
            </button>
          </div>
        </div>
